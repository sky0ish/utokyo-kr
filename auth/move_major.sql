...
--     [문과] 법학·공공정책(L) · 경제학부(EC) · 인문·사회·교육(HS) · 교양·총합문화(T)
--            기타(NONE)
--
--   무엇을 옮기나
...
select category as "지금 게시판",
       case
         ...
         when title ~ 'C\.?U\.?E|도시공학|도시|토목|환경|사회기반|방재|신영역' then '도시·토목·환경'
         when title ~ '금속|재료|공업화학|화학시스템|화공|신소재|응용화학' then '화학·응용화학·재료'
         when title ~ '의학|의약|약학|의대|병원|보건|간호|약우회|의사회|치과|수의' then '의학·약학·간호학'
...
update public.posts p
   set category = 'major',
       title = '[' || case
         ...
         when t.title ~ 'C\.?U\.?E|도시공학|도시|토목|환경|사회기반|방재|신영역' then '도시·토목·환경'
         when t.title ~ '금속|재료|공업화학|화학시스템|화공|신소재|응용화학' then '화학·응용화학·재료'
         when t.title ~ '의학|의약|약학|의대|병원|보건|간호|약우회|의사회|치과|수의' then '의학·약학·간호학'
