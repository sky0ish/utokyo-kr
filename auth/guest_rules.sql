-- =========================================================
--  비동문 준회원(Guest) 이 무엇을 할 수 있는지 정합니다.
--
--   보기   포럼·세미나 · 구인·채용  두 게시판만
--   쓰기   구인·채용 한 곳만 (글도 댓글도)
...
-- ── ① 지금 보는 사람이 준회원인가 ──
--    등급이 'guest' 이거나 구분이 'GUEST' 이면 준회원으로 봅니다.
--    운영진은 준회원으로 보지 않습니다.
...
create policy "guest write limit" on public.posts as restrictive for insert
  with check (not public.is_guest() or category = 'jobs');
...
create policy "guest comment limit" on public.comments as restrictive for insert
  with check (
    ...
    or exists (select 1 from public.posts p
                where p.id = comments.post_id and p.category = 'jobs')
  );
...
select unnest(public.guest_cats()) as "준회원이 보는 게시판";
