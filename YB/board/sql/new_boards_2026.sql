...
--   게시판   > 장학·연구지원 (scholarship) — 장학금·연구비·연구계획서 (준회원부터 열람)
...
select '도쿄대학 한국인학생회', 'YB', 'scholarship',
       '[기타] 장학·연구지원 게시판을 엽니다',
       '유학생이 넣을 수 있는 장학금과 연구비, 그리고 그것을 받기 위해 쓴 연구계획서를 함께 나누는 곳입니다.' || chr(10) ||
       '받으신 분, 떨어지신 분 모두 겪은 이야기를 남겨주시면 다음 사람에게 큰 도움이 됩니다.' || chr(10) || chr(10) ||
       '말머리는 교내 · 일본정부 · 한국정부 · 민간재단 · 학진(JSPS) · 연구계획서 · 기타 입니다.' || chr(10) || chr(10) ||
       '■ 장학금·연구비를 알리실 때' || chr(10) ||
       '  · 지원 자격 (학년·국적·성적 조건)' || chr(10) ||
       '  · 마감일과 신청 방법' || chr(10) ||
       '  · 금액과 기간' || chr(10) ||
       '이 세 가지만 적어주셔도 충분합니다.' || chr(10) || chr(10) ||
       '■ 학진(JSPS)' || chr(10) ||
       '특별연구원(DC1·DC2·PD)과 과연비(科研費)처럼 해마다 돌아오는 사업은' || chr(10) ||
       '작년 일정과 후기가 그대로 도움이 됩니다. 학진 말머리로 모아 주세요.' || chr(10) || chr(10) ||
       '■ 연구계획서를 나눠 주실 때' || chr(10) ||
       '실제로 낸 계획서만큼 좋은 길잡이가 없습니다. 다만 두 가지만 지켜 주세요.' || chr(10) ||
       '  · 지도교수님과 공동연구자의 동의를 먼저 받아 주세요.' || chr(10) ||
       '  · 아직 발표하지 않은 아이디어나 미공개 데이터는 지우고 올려 주세요.' || chr(10) ||
       '    (회원 전용이지만 화면은 얼마든지 옮겨질 수 있습니다)' || chr(10) ||
       '통과한 계획서든 떨어진 계획서든 모두 배울 것이 있습니다.' || chr(10) || chr(10) ||
       '※ 마감이 지난 글도 지우지 말아주세요. 이듬해 같은 시기에 다시 열리는 것이 많습니다.' || chr(10) ||
